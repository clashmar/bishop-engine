...
impl DynamicTilemapUiElement for ResizeButton {
    ...
    fn on_click(
        ...
        let (mut delta_pos, mut proposed_size) = match self.action {
            ResizeAction::AddTop => (
                vec2(0.0, -1.0),
                vec2(map.width as f32, map.height as f32 + 1.0),
            ),
            ...
            ResizeAction::AddLeft => (
                vec2(-1.0, 0.0),
                vec2(map.width as f32 + 1.0, map.height as f32),
            ),
        ...
        delta_pos *= grid_size;
        proposed_size *= grid_size;

        // Check for overlaps
        let proposed_pos = *room_position + delta_pos;

        if coord::overlaps_existing_rooms(proposed_pos, proposed_size, other_bounds, grid_size) {
            // Skip the resize
            return;
        }
...
impl ResizeButton {
    /// Build all resize buttons for a tilemap.
    pub fn build_all(
        map: &TileMap,
        ...
        let margin: f32 = grid_size / 8.0;
        let btn_size: Vec2 = vec2(grid_size / 1.75, grid_size / 1.75);
        let outer_gap: f32 = grid_size * 3.;
        let inner_gap: f32 = grid_size * 2.;
