    ...
    pub struct AnimationModule {
        ...
        sprite_btn_id: WidgetId,
        frame_x_id: WidgetId,
        frame_y_id: WidgetId,
        cols_id: WidgetId,
        rows_id: WidgetId,
        fps_id: WidgetId,
        loop_chk_id: WidgetId,
    ...
    impl Default for AnimationModule {
        fn default() -> Self {
            Self {
                ...
                rename_initial_value: String::new(),
                warning: None,
                rename_field_id: WidgetId::fresh(),
                sprite_btn_id: WidgetId::fresh(),
                frame_x_id:WidgetId::fresh(),
                frame_y_id:WidgetId::fresh(),
                cols_id: WidgetId::fresh(),
                rows_id: WidgetId::fresh(),
                fps_id: WidgetId::fresh(),
                loop_chk_id: WidgetId::fresh(),
                offset_x_id: WidgetId::fresh(),
                offset_y_id: WidgetId::fresh(),
                select_dropdown_id: WidgetId::fresh(),
                set_dropdown_id: WidgetId::fresh(),
            }
        }
    }
    ...
    impl InspectorModule for AnimationModule {
        ...
        fn draw(
            ...
            if gui_button(btn_rect, ADD_LABEL) {
                ...
                animation.clips.insert(new_id.clone(), Clip::default());
            ...
            draw_current_clip_dropdowns(
                ...
                animation, 
                all_ids,
                &mut self.pending_rename,
                &mut self.rename_initial_value,
                &mut self.warning,
                &mut self.select_dropdown_id,
                &mut self.set_dropdown_id,
            );
    ...
    pub fn draw_current_clip_dropdowns(
        ...
        all_ids: Vec<ClipId>,
        pending_rename: &mut bool,
        rename_initial_value: &mut String,
        warning: &mut Option<WarningToast>,
        select_dropdown_id: &mut WidgetId,
        set_dropdown_id: &mut WidgetId,
    ) {
        ...
        if let Some(chosen) = chosen {
            match chosen {
                ...
                ClipId::New => {
                    *pending_rename = true;
                    rename_initial_value.clear();          
                    return;
                }
                ClipId::Custom(name) => {
                    *pending_rename = true;
                    *rename_initial_value = name.clone();   
                    return;
                }
        ...
        if *pending_rename {
            ...
            println!("{rename_initial_value}");
            let (entered, focused) = gui_input_text_clamped_focused(input_rect, &rename_initial_value, CLAMP);

            // Reset or Rename when Enter is pressed
            if !focused {
                gui_input_text_reset(input_rect);
                *pending_rename = false;
    ...
    pub fn draw_frame_size_fields(y: f32, rect: Rect, clip: &mut Clip) {
        ...
        draw_text(LABELS[0], lbl_x.x, lbl_x.y + 15.0, 18.0, WHITE);
        draw_text(LABELS[1], lbl_y.x, lbl_y.y + 15.0, 18.0, WHITE);

        // Numeric inputs
        clip.frame_size.x = gui_input_number(inp_x, clip.frame_size.x);
        clip.frame_size.y = gui_input_number(inp_y, clip.frame_size.y);
    }
    ...
    pub fn draw_spritesheet_dimension_fields(y: f32, rect: Rect, clip: &mut Clip) {
        ...
        draw_text(LABELS[0], lbl_c.x, lbl_c.y + 15.0, 18.0, WHITE);
        draw_text(LABELS[1], lbl_r.x, lbl_r.y + 15.0, 18.0, WHITE);

        clip.cols = gui_input_number(inp_c, clip.cols as f32) as usize;
        clip.rows = gui_input_number(inp_r, clip.rows as f32) as usize;
    }
    ...
    pub fn draw_fps_and_loop(y: f32, rect: Rect, clip: &mut Clip) {
        ...
        draw_text(LABELS[0], lbl_fps.x, lbl_fps.y + 15.0, 18.0, WHITE);
        draw_text(LABELS[1], lbl_loop.x, lbl_loop.y + 15.0, 18.0, WHITE);

        clip.fps = gui_input_number(inp_fps, clip.fps);
    ...
    pub fn draw_offset_fields(y: f32, rect: Rect, clip: &mut Clip) {
        ...
        clip.offset.x = gui_input_number(inp_x, clip.offset.x);
        clip.offset.y = gui_input_number(inp_y, clip.offset.y);
    }

    /// Returns every ClipId that has a concrete Clip stored in the map.
    fn existing_clip_ids(clips: &HashMap<ClipId, Clip>) -> Vec<ClipId> {
