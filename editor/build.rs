...
fn generate_lua_components() {
    ...
    fs::create_dir_all(&out_dir).expect("cannot create .engine folder");
...
/// Generates Rust code that embeds all .lua files from the .engine directory.
fn generate_engine_scripts_rs() {
    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    let engine_dir = manifest_dir.join("scripts").join(".engine");
    ...
    for filename in &entries {
        rust.push_str(&format!(
            "    (\"{}\", include_str!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/scripts/.engine/{}\"))),\n",
            filename, filename
        ));
    }
    ...
    // Rerun if .engine directory changes
    println!("cargo:rerun-if-changed=scripts/.engine");
}